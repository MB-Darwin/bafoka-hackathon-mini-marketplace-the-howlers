...
class BotService {
  ...
  constructor() {
    ...
    this.COMMUNITIES = {
      ...
      BATOUFAM: {
        name: "BATOUFAM",
        voucher: "MBIP TSWEFAP",
        displayName: "🏘️ BATOUFAM",
      },
      FONDJOMEKWET: {
        name: "FONDJOMEKWET",
        voucher: "MBAM",
        displayName: "🏘️ FONDJOMEKWET",
      },
    };
  ...
  public async processIncomingMessage(
    messageData: IncomingMessageData & Record<string, any>
  ): Promise<void> {
    try {
      // Normalize phone number
      const phoneNumber: string | undefined =
        messageData.from || messageData.From || messageData.WaId;
      if (!phoneNumber) {
        logger.error(
          { messageData },
          "No phone number found in incoming message."
        );
        return;
      }
      ...
      let userSession: UserSession | null =
        await this.userSessionService.getSession(phoneNumber);
      if (!userSession) {
      ...
      if (interactive) {
        return await this.handleInteractiveResponse(
          phoneNumber,
          interactive,
          userSession
        );
      ...
      logger.info(
        { messageData },
        "Received message with no text or interactive content."
      );
    } catch (error) {
    ...
    try {
      switch (responseId) {
        case "seller_register":
          return await this.initiateCommunitySelection(
            phoneNumber,
            userSession,
            "seller"
          );
        ...
        case "select_bameka":
          return await this.handleCommunitySelection(
            phoneNumber,
            userSession,
            "BAMEKA"
          );
        case "select_batoufam":
          return await this.handleCommunitySelection(
            phoneNumber,
            userSession,
            "BATOUFAM"
          );
        case "select_fondjomekwet":
          return await this.handleCommunitySelection(
            phoneNumber,
            userSession,
            "FONDJOMEKWET"
        ...
        case "back":
          return await this.showMainMenu(phoneNumber);
        default:
          return await this.handleUnknownResponse(phoneNumber);
      }
    } catch (error) {
      logger.error(
        { error },
        `Error handling interactive response ${responseId}:`
      );
      throw new BotServiceError(`Failed to process ${responseId}`, {
        cause: error,
      });
    }
  }
    ...
    try {
      ...
      switch (userSession.currentFlow) {
        case this.FLOWS.MAIN_MENU:
          return await this.handleMainMenuInput(
            phoneNumber,
            message,
            userSession
          );
        ...
        case this.FLOWS.COMMUNITY_SELECTION:
          return await this.handleCommunitySelectionInput(
            phoneNumber,
            message,
            userSession
          );
        ...
        case this.FLOWS.SELLER_MENU:
          return await this.handleSellerMenuInput(
            phoneNumber,
            message,
            userSession
          );
        ...
        case this.FLOWS.BUYER_MENU:
          return await this.handleBuyerMenuInput(
            phoneNumber,
            message,
            userSession
          );
        ...
        case this.FLOWS.SHOP_CONFIRMATION:
          return await this.handleShopCreationFlow(
            phoneNumber,
            message,
            userSession
          );
      ...
      logger.error(
        { error },
        `Error handling text message in flow ${userSession.currentFlow}:`
      );
  ...
  /** -------------------- COMMUNITY SELECTION -------------------- */
  private async initiateCommunitySelection(
    phoneNumber: string,
    userSession: UserSession,
...
Reply with the number of your choice.`;
  ...
  private async handleCommunitySelectionInput(
    phoneNumber: string,
    message: string,
    userSession: UserSession
  ): Promise<void> {
  ...
  private async handleCommunitySelection(
    phoneNumber: string,
    userSession: UserSession,
    ...
    const userType =
      pendingUserType === "seller" || pendingUserType === "buyer"
        ? pendingUserType
        : null;
    ...
    const updatedSession: Partial<UserSession> = {
      ...
      userType,
      currentFlow:
        userType === "seller" ? this.FLOWS.SELLER_MENU : this.FLOWS.BUYER_MENU,
      userData: {
    ...
    try {
      ...
      if (!existingUser) {
        await this.whatsappService.sendMessage(
          phoneNumber,
          `🎁 You have ${voucherBalance} ${community.voucher} to start shopping!`
        );
      }

      if (userType === "seller") {
        return await this.showSellerMenu(
          phoneNumber,
          updatedSession as UserSession
        );
      } else {
        return await this.showBuyerMenu(
          phoneNumber,
          updatedSession as UserSession
        );
      }
      ...
      await this.whatsappService.sendMessage(
        phoneNumber,
        "❌ Could not save your community selection. Try again."
      );
    }
  }

  /** -------------------- SELLER MENU -------------------- */
  private async showSellerMenu(
    phoneNumber: string,
    userSession: UserSession
  ): Promise<void> {
    console.log("first", userSession);
    const community =
      userSession.userData?.communityDisplayName || "your community";
  ...
  async handleSellerMenuInput(
    ...
    switch (message) {
      ...
      case "3":
        await this.whatsappService.sendMessage(
          phoneNumber,
          "💳 Setup Payments feature coming soon!"
        );
      ...
      case "4":
        await this.whatsappService.sendMessage(
          phoneNumber,
          "🔗 EOS Account setup feature coming soon!"
        );
        return await this.showSellerMenu(phoneNumber, userSession);
...
Type "cancel" to return to the menu.`
      ...
      await this.whatsappService.sendErrorMessage(
        phoneNumber,
        "Could not start shop creation. Please try again."
      );
    }
  }

  async handleShopCreationFlow(
    phoneNumber: string,
    message: string,
    userSession: UserSession
  ): Promise<void> {
    ...
    try {
      switch (userSession.currentFlow) {
        case this.FLOWS.SHOP_NAME_INPUT:
          return await this.handleShopNameInput(
            phoneNumber,
            message,
            userSession
          );
        ...
        case this.FLOWS.SHOP_DESCRIPTION_INPUT:
          return await this.handleShopDescriptionInput(
            phoneNumber,
            message,
            userSession
          );
        ...
        case this.FLOWS.SHOP_CATEGORY_SELECT:
          return await this.handleShopCategorySelect(
            phoneNumber,
            message,
            userSession
          );
        ...
        case this.FLOWS.SHOP_CONFIRMATION:
          return await this.handleShopConfirmation(
            phoneNumber,
            message,
            userSession
          );
  ...
  async handleShopNameInput(
    phoneNumber: string,
    name: string,
    userSession: UserSession
  ): Promise<void> {
    console.log("first", phoneNumber + name);
    // Validate name
    ...
    const nameTaken = await shopService.isShopNameTaken(
      name.trim(),
      userSession.community!
    );
    if (nameTaken) {
      await this.whatsappService.sendMessage(
        phoneNumber,
...
Type "back" to change the shop name.`
  ...
  async handleShopDescriptionInput(
    phoneNumber: string,
    description: string,
    userSession: UserSession
  ): Promise<void> {
...
Type "back" to change description.`
  ...
  async handleShopCategorySelect(
    phoneNumber: string,
    input: string,
    userSession: UserSession
  ): Promise<void> {
...
Type your choice:`
  ...
  async createShopFinal(
    phoneNumber: string,
    userSession: UserSession
  ): Promise<void> {
    try {
      await this.whatsappService.sendProcessingMessage(
        phoneNumber,
        "creating your shop"
      );
...
Shop management features coming soon!`
      ...
      await this.whatsappService.sendErrorMessage(
        phoneNumber,
        "Could not load shop information."
      );
    }
  }

  /** -------------------- BUYER MENU -------------------- */
  private async showBuyerMenu(
    phoneNumber: string,
    userSession: UserSession
  ): Promise<void> {
    console.log("first", userSession);
...
Reply with the number of your choice.`;
    ...
    console.log("second", menuText);

    await this.whatsappService.sendMessage(phoneNumber, menuText);
  }

  private async handleBuyerMenuInput(
    phoneNumber: string,
    message: string,
    userSession: UserSession
  ): Promise<void> {
    switch (message) {
      case "1":
        await this.whatsappService.sendMessage(
          phoneNumber,
          "🏪 Browse Shops feature coming soon!"
        );
      ...
      case "2":
        await this.whatsappService.sendMessage(
          phoneNumber,
          "📦 View Products feature coming soon!"
        );
        return await this.showBuyerMenu(phoneNumber, userSession);
...
Type "help" for assistance.`;
  ...
  private async initializeSellerFlow(
    phoneNumber: string,
    userSession: UserSession
  ): Promise<void> {
    return await this.showSellerMenu(phoneNumber, userSession);
  ...
  private async initializeBuyerFlow(
    phoneNumber: string,
    userSession: UserSession
  ): Promise<void> {
    return await this.showBuyerMenu(phoneNumber, userSession);
  ...
  private async handleCommunitySelectionText(
    phoneNumber: string,
    message: string,
    userSession: UserSession
  ): Promise<void> {
